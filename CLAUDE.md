...
AM（Agent Manager）是一套在本機執行的網站，用來管理 Claude Code 的連線設定檔（訂閱制、各家 API key 服務商），搭配終端機的 `am` 指令選擇要用哪個設定啟動 Claude Code，每個終端視窗可以各用不同設定。網站與 `am` 指令一起安裝，網站開機自動在背景執行，並提供給其他人安裝使用。
